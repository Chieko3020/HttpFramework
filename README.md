...
> 环境：2 vCPU / 1968 MB 的 VPS（**压测端与被测服务同机 loopback**），服务线程池 4，`wrk 4.2.0`
> 测量脚本：`scripts/bench_http.sh`（HTTP，`wrk -t4`）、`run_http_bench.sh feature/WebSocket`（HTTP 分项，`wrk -t2`）、
> `scripts/run_wss_bench.sh` 与 `scripts/bench_wss_concurrent.sh`（WSS）；原始输出在 `results/` 与 `results/wss/`
> 两套 HTTP 脚本口径不同，数字不可互比。

### 并发梯度（长连接，`wrk -t4`）

| 并发 | 吞吐 (req/s) | p50 | p99 | socket 错误 |
|---|---|---|---|---|
| 100 | 34,776 | 2.73 ms | 6.07 ms | 0 |
| 500 | 33,160 | 14.70 ms | 25.10 ms | 0 |
| 1000 | 33,305 | 29.33 ms | 50.00 ms | 0 |
| 2000 | 30,822 | 64.63 ms | 100.65 ms | 0 |
| 5000 | 32,657 | 147.52 ms | 223.70 ms | 0 |

**读法**：5,000 并发下保持 3.2 万 req/s、零 socket 错误——socket 错误列必须与吞吐一起给出，
连接失败与读超时都记在这一列。延迟随并发近似线性增长（符合 `延迟 ≈ 并发 ÷ 吞吐` 的排队关系），
所以"低延迟"与"高并发"必须**同时标注档位**才有意义。

### 每请求 CPU

| 指标 | 数值 |
|---|---|
| 每请求 CPU（c=1000，`/proc/<pid>/stat` 差分 ÷ 请求数） | **33.9 µs**（3,651 jiffies / 1,078,219 请求） |

### 路由与中间件（`-t4`，c=20）

`bench_server --middleware 50` / `--routes 500`，原始输出 `results/wss/opt_c20_20260921.txt`：

| 场景 | 吞吐 (req/s) | p50 |
|---|---|---|
| 50 层中间件 | 26,155 | 0.71 ms |
| 500 条路由、命中**末条** | 34,705 | 0.54 ms |
| 500 条路由、命中靠前 | 34,736 | 0.54 ms |

根因是每个请求都把中间件链与路由表**整表重算**（`findMiddlewares` 走 O(M) 正则、路由逐条匹配）。
改动是入口处一次性算好中间件列表，路由改为 method 索引 + 静态哈希 + 分段匹配。
"命中靠前与命中末条几乎同速"说明收益来自"**找得对不对**"，而不是"找得快"。

### 内存池：无可观测收益

交替执行（默认 ↔ `--mempool`），各 30 秒（`-t4`，c=1000）：
...
三轮方向不一致（第 2 轮开池慢、第 1、3 轮开池快），差异落在同批测量的波动范围内，
...
| 档位 | 吞吐 |
|---|---|
| 消息往返 256 B | 8,933 msg/s |
| 消息往返 1 KB | 7,240 msg/s |
| 消息往返 16 KB | 4,139 msg/s |
...
单位相同，因此**可以直接相比**：总吞吐从 1 条连接的 8,230 msg/s（3 轮中位）涨到 32 条连接的
21,025 msg/s，即 **2.55 倍**；同期每条连接自己的速率掉到 `21,025 ÷ 32 ≈ 657 msg/s`。
**总吞吐上涨是拿每条连接的速率换来的**：并发让单条变慢、总量变快，这正是必须把两个口径
分开写的原因（只写一个数，会被读成另一种意思）。

| 并发连接 | 总吞吐（合计） | 轮间波动 | p50 | p99 | 相对单连接 |
|---|---|---|---|---|---|
| 1 | 8,230 msg/s | 5.9% | 0.099–0.111 ms | 0.147–0.218 ms | 1.00x |
| 8 | 19,536 msg/s | 17.8% | 0.31–0.41 ms | 0.65–0.91 ms | 2.37x |
| 32 | **21,025 msg/s** | 13.2% | 1.28–1.58 ms | 2.04–3.53 ms | 2.55x |
| 64 | 18,673 msg/s | 5.1% | 2.84–3.28 ms | 5.08–8.06 ms | 2.27x |

- **平台约 1.9–2.1 万 msg/s，8 条连接就到达**：8/32/64 三档中位数是 19,536 / 21,025 / 18,673，
  彼此差异与各自的轮间波动同量级，所以不给"哪个并发档更快"这类结论——瓶颈是 2 vCPU 而非连接数。
- **延迟随并发近线性增长，是排队不是退化**：p50 从 0.10 ms 涨到 3 ms 量级（约 30 倍），
  而这个区间里吞吐没有同比例恶化。
- **口径**：客户端与服务端同机，客户端同样吃这 2 个 vCPU，因此这是双端同机的合计值，
  不等于纯服务端能力上限。

原始输出见 `results/wss_concurrent_20260921_3runs.txt`。

### 短连接

三次测量是 12,033 / 9,791 / 8,622 req/s。短连接吞吐受客户端端口与 TIME_WAIT 资源影响、
波动可达 30%+，**只作区间参考（约 0.9–1.2 万 req/s）**，不参与任何结论。
