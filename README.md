...
> 两套脚本口径不同、数字不可互比：`scripts/bench_http.sh`（`wrk -t4`，并发 100–5000，各 30 s）与
> `scripts/run_http_bench.sh main results/main`（`wrk -t2`，每项独立启动服务，另出 `results/main/REPORT.md`）
> 原始输出：`results/http_bench_20260921_0254.txt`、`results/main/`

### 并发梯度（长连接，`wrk -t4`）

| 并发 | 吞吐 (req/s) | p50 | p99 | socket 错误 |
|---|---|---|---|---|
| 100 | 35,858 | 2.62 ms | 6.61 ms | 0 |
| 500 | 36,039 | 13.44 ms | 23.97 ms | 0 |
| 1000 | 31,658 | 30.84 ms | 51.57 ms | 0 |
| 2000 | 27,608 | 70.46 ms | 132.08 ms | 0 |
| 5000 | **29,236** | 169.60 ms | 231.39 ms | 0 |

**读法**：5,000 并发下保持 2.9 万 req/s 量级、零 socket 错误。p50 从 100 档的 2.62 ms 升到 5000 档的
169.60 ms，量级符合 `延迟 ≈ 并发 ÷ 吞吐` 的排队关系，所以"低延迟"与"高并发"必须**同时标注档位**才有意义。

### 每请求 CPU

| 口径 | 数值 |
|---|---|
| `bench_http.sh`（`-t4`，c=1000；`/proc/<pid>/stat` 差分 ÷ 请求数） | **37.9 µs**（3,667 jiffies / 967,885 请求） |
| `run_http_bench.sh` A6 项（`-t2`，c=500） | **29.6 µs**（3,616 jiffies / 1,102,625 请求） |

### 路由与中间件（`-t4`，c=20）

`bench_server --middleware 50` / `--routes 500`，原始输出 `results/main/opt_c20_20260921.txt`：

| 场景 | 吞吐 (req/s) | p50 |
|---|---|---|
| 50 层中间件 | 23,855 | 0.77 ms |
| 500 条路由、命中**末条** | 32,901 | 0.58 ms |
| 500 条路由、命中靠前 | 33,762 | 0.56 ms |
...
分项脚本同批（`-t2`）：无池 36,271 → 开池 39,074 req/s。两者差异落在同批测量的波动范围内，
...
三次测量是 10,533 / 12,864 / 13,737 req/s。短连接吞吐受客户端端口与 TIME_WAIT 资源影响、
波动可达 30%+，**只作区间参考（约 1.1–1.4 万 req/s）**，不参与任何结论。
