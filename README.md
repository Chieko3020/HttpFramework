...
两批交替复测的方向并不一致（`0639` 批三轮翻转，`2200`/`2220` 批三轮一致偏低）。
**结论是默认关闭**——理由是无收益 + 多占约 20 MB，而不是"性能更差"。
...
> 测量脚本：`scripts/bench_http.sh`（HTTP）与 `scripts/bench_wss_concurrent.sh`（WSS 并发）——前者启动服务后会**校验监听者身份**，后者先探测服务端口，结果落盘 `results/`
> 下表是 2026-09-20 22:20 用该脚本重跑的一批，原始输出为 `results/http_bench_20260920_2220.txt`。与更早批次相比，每请求 CPU 与短连接吞吐有差异（机器状态不同），各批数据都保留在 `results/`

### 并发梯度（长连接）

| 并发 | 吞吐 (req/s) | p50 | p99 | socket 错误 |
|---|---|---|---|---|
| 100 | 37,520 | 2.54 ms | 5.43 ms | 0 |
| 500 | 36,658 | 13.24 ms | 23.63 ms | 0 |
| 1000 | 34,625 | 27.97 ms | 52.12 ms | 0 |
| 2000 | 30,252 | 65.58 ms | 102.80 ms | 0 |
| 5000 | 29,287 | 168.72 ms | 231.30 ms | **read 7** |

**读法**：多次测量落在 **29k–38k** 区间，本批从 100 档的 37.5k 降到 5000 档的 29.3k。
**5,000 档不能写成"零 socket 错误"**：这批出现 `read 7`（约 878,600 个请求里 7 个，量级极小，
但引用时必须连同错误列一起给出），其余四档为 0。
...
| 指标 | 数值 |
|---|---|
| 每请求 CPU（c=1000，`/proc/<pid>/stat` 差分 ÷ 请求数） | **37.7 µs**（3704 jiffies / 981,462 请求） |

同一方法在更早两批测到 **37.1 µs**（`results/http_bench_20260920_0639.txt`）与 **32.3 µs**（`results/historical-20260918-19/retest-hf.log`：3244 jiffies ÷ 1,005,203 请求 = 32.27 µs，c=1000）——散布来自机器状态而非代码改动。该历史批的原始日志原先只在 `/tmp`，已于 2026-09-21 补入 `results/historical-20260918-19/`（连同 `scripts/clean-retest*.sh`），从此可被引用。
...
本批**三轮方向一致**（开池低 1.6%–8.5%），而上一批三轮方向相反（`results/http_bench_20260920_0639.txt`）。
跨批方向不稳定 ⇒ 只能表述为**无可观测收益**，不给加速比；单看本批，开池也没有任何一轮更快。
...
分成两组看：**单连接往返速率**回答"一条连接上的延迟下限"，**并发总吞吐**回答"服务端同时扛住多少"。
两者口径不同，不能互相替代。

**一、单连接往返速率**——用 `examples/wss_echo`（回声服务）与 `examples/wss_bench_client`
（逐条发送并等回显、记录往返延迟）测得：

| 档位 | 吞吐 |
|---|---|
| 消息往返 256 B | 8,857 msg/s |
| 消息往返 1 KB | 7,935 msg/s |
| 消息往返 16 KB | 3,856 msg/s |
| TLS 握手 | 18.9 次/秒（50 成功 / 0 失败） |
| 并发连接 | 100/100 建立成功（判据：升级响应首行匹配 `^HTTP/1.[01] 101`） |

16 KB 档低于小包是预期的（超过 MSS 会被拆成十几段），但仍保持 3.9 千 msg/s 量级。
握手一项含 `openssl s_client` 的进程启动开销，是弱指标，只作量级参考。
原始输出见 `results/wss/`（`d1_throughput_*.txt`、`d2_handshake.txt`、`d3_maxconn.txt`）。

**二、并发总吞吐**——用 `scripts/bench_wss_concurrent.sh` 并发起 N 条连接、每条各自串行往返，
3 轮独立复现取中位数（1 KB 消息、每条连接 2000 条、2 vCPU 本机回环）。

下表「总吞吐」指 **N 条连接的合计**：各连接完成的消息数之和 ÷ 墙钟时间。它与上面单连接那一组
单位相同，因此**可以直接相比**：总吞吐从 1 条连接的 7,407 msg/s（3 轮中位）涨到 8 条连接的
19,560 msg/s，即 **2.64 倍**。而同期每条连接自己的速率掉到 `19,560 ÷ 8 ≈ 2,445 msg/s`，
不到单连接的三分之一。**总吞吐涨 2.64 倍，是拿每条连接的速率换来的**：并发让单条变慢、总量变快，
这正是必须把两个口径分开写的原因（只写一个数，会被读成另一种意思）。

算倍数时**基线取同批的 1 连接档（7,407）**，但它在本批的轮间波动达 **22.9%**
（7,194 / 7,407 / 8,889——每轮只有 2000 条消息、约 0.27 秒），超过「波动 >20% 的指标不单独发布」
的纪律，所以这个倍数**只作量级参考**：换 D1 批的单连接值 7,935 作分母是 2.46x，换本批最高那一轮
8,889 则是 2.20x。三种取法都落在"2 倍出头到 2.6 倍"，但不要把 2.64 当精确值引用。

| 并发连接 | 总吞吐（合计） | 轮间波动 | p50 | p99 | 相对单连接 |
|---|---|---|---|---|---|
| 1 | 7,407 msg/s | **22.9%** | 0.116 ms | 0.281 ms | 1.00x |
| 8 | **19,560 msg/s** | 5.6% | 0.334 ms | 0.779 ms | 2.64x |
| 32 | 18,423 msg/s | 18.1% | 1.591 ms | 2.810 ms | 2.49x |
| 64 | 18,741 msg/s | 3.9% | 3.085 ms | 5.567 ms | 2.53x |

- **平台约 1.9–2.0 万 msg/s，8 条连接就到达**：8/32/64 三档中位数是 19,560 / 18,423 / 18,741，
  彼此差异小于各自的轮间波动，所以不给"哪个并发档更快"这类比较结论——瓶颈是 2 vCPU 而非连接数。
- **加速比量级约 2.5 倍**（单连接 7.4–7.9 千 → 平台 1.9 万），与 2 个 vCPU 同数量级。
  上一批（`0757`）用同一方法得到 2.23–2.31x，差别主要来自 1 连接档的波动，平台档两批都是 1.9–2.0 万。
- **延迟随并发近线性增长，是排队不是退化**：p50 从 0.116 ms 涨到 3.085 ms（约 27 倍），而这个区间里吞吐没有同比例恶化。
- **"哪一档最稳"本身也在批间漂移**：本批 8 档 5.6%、64 档 3.9% 最稳、32 档 18.1% 处在边界，
  而上一批是 32 档 4.5% 最稳、8 档 16.4%——引用单档数字时应按当批波动判断，不要沿用固定偏好。
- **口径**：客户端与服务端同机，客户端同样吃这 2 个 vCPU，因此这是双端同机的合计值，不等于纯服务端能力上限。

原始输出见 `results/wss_concurrent_20260920_2239_3runs.txt`，读数说明见
`results/wss_concurrent_20260920_2239_summary.md`。

### 短连接

三次测量是 10,558 / 12,285 / 12,652 req/s。短连接吞吐受客户端端口与 TIME_WAIT 资源影响、
波动可达 30%+，**只作区间参考（约 1.1–1.3 万 req/s）**，不参与任何结论。
...
bash scripts/bench_wss_concurrent.sh 32 2000 1024   # WSS 并发总吞吐：[连接数] [每条消息数] [字节数]
```

HTTP 脚本启动服务后会校验监听者身份，不通过则把整批标记作废；WSS 脚本会先探测服务端口，
服务不在就判定不产出数据（避免留下一批"全失败"的假结果）。结果统一写入 `results/`。
WSS 并发脚本可用 `WSS_OUT=path` 指定输出文件，多轮复现追加到同一文件便于直接看轮间波动。
