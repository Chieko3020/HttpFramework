...
单位相同，因此**可以直接相比**：总吞吐从 1 条连接的 8,511 msg/s 涨到 32 条连接的 19,656 msg/s，
即 **2.31 倍**。而同期每条连接自己的速率掉到 `19,656 ÷ 32 ≈ 614 msg/s`，不到单连接的十四分之一。
**总吞吐涨 2.31 倍，是拿每条连接的速率换来的**：并发让单条变慢、总量变快，这正是必须把两个
口径分开写的原因（只写一个数，会被读成另一种意思）。

算倍数时**基线取同批的 1 连接档（8,511）**。它与上面单连接表里的 8,919 是**同一装置**
（`bench_server` + `wss_bench_client` + `/echo`）下的不同批次，差 4.6%（1 KB 档单连接速率在两批里
落在 8.5–8.9 千）；若拿 8,919 当分母会得到 2.2 倍，结论不变但基线不齐。

| 并发连接 | 总吞吐（合计） | 轮间波动 | p50 | p99 | 相对单连接 |
|---|---|---|---|---|---|
| 1 | 8,511 msg/s | 9.3% | 0.098 ms | 0.214 ms | 1.00x |
| 8 | 18,957 msg/s | 16.4% | 0.368 ms | 0.978 ms | 2.23x |
| 32 | 19,656 msg/s | 4.5% | 1.422 ms | 2.816 ms | 2.31x |
| 64 | 18,254 msg/s | 5.4% | 3.170 ms | 5.869 ms | 2.14x |

- **平台约 2 万 msg/s，8 条连接就到达**：8→32 条连接吞吐不再增长，瓶颈是 2 vCPU 而非连接数。
- **加速比封顶约 2.3 倍**（单连接 8.5 千 → 平台 2 万），与 2 个 vCPU 同数量级；64 条连接回落到 2.14x。
- **延迟随并发近线性增长，是排队不是退化**：p50 从 0.098 ms 涨到 3.17 ms（32 倍），而这个区间里吞吐没有同比例恶化。
- **8 档波动 16.4%、32 档仅 4.5%**：8 条连接处于"刚吃饱"的临界区，调度抖动直接反映到总时长；引用单档数字时用 32 档。
- **口径**：客户端与服务端同机，客户端同样吃这 2 个 vCPU，因此这是双端同机的合计值，不等于纯服务端能力上限。

原始输出见 `results/wss_concurrent_20260920_0757_3runs.txt`，读数说明见
`results/wss_concurrent_20260920_summary.md`。
