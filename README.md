...
> 下表是 2026-09-20 22:00 用该脚本重跑的一批，原始输出为 `results/http_bench_20260920_2200.txt`。与更早批次相比，每请求 CPU 与短连接吞吐有差异（机器状态不同）

### 并发梯度（长连接）

| 并发 | 吞吐 (req/s) | p50 | p99 | socket 错误 |
|---|---|---|---|---|
| 100 | 35,191 | 2.67 ms | 7.05 ms | 0 |
| 500 | 36,804 | 12.98 ms | 26.33 ms | 0 |
| 1000 | 34,720 | 27.76 ms | 50.81 ms | 0 |
| 2000 | 29,220 | 67.92 ms | 103.57 ms | 0 |
| 5000 | **35,227** | 137.54 ms | 209.79 ms | 0 |

**读法**：多次测量落在 **29k–38k** 区间。本批 500 档最高 36.8k、2000 档掉到 29.2k、5000 档又回到 35.2k——
档间非单调说明**单批内存在机器噪声**（压测端与被测服务同机，两端抢同一批 CPU），所以只下一条结论：
**5,000 并发下仍保持 3 万 req/s 量级、零 socket 错误**，不对档间差异做因果解读。
p50 从 100 档的 2.67 ms 涨到 5000 档的 137.54 ms，量级上仍符合 `延迟 ≈ 并发 ÷ 吞吐` 的排队关系，
...
| 指标 | 数值 |
|---|---|
| 每请求 CPU（c=1000，`/proc/<pid>/stat` 差分 ÷ 请求数） | **32.5 µs**（3669 jiffies / 1,129,918 请求） |
...
本批**三轮方向一致**（开池低 4.7%–6.2%），而上一批三轮方向相反（`results/http_bench_20260920_0639.txt`）。
...
三次测量是 12,664 / 11,518 / 13,992 req/s。短连接吞吐受客户端端口与 TIME_WAIT 资源影响、
波动可达 30%+，**只作区间参考（约 1.2–1.4 万 req/s）**，不参与任何结论。
