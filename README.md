...
分成两组看：**单连接往返速率**回答"一条连接上的延迟下限"，**并发聚合吞吐**回答"服务端同时扛住多少"。
两者口径不同，不能互相替代。
...
**二、并发聚合吞吐**——用 `scripts/bench_wss_concurrent.sh` 并发起 N 条连接、每条各自串行往返，
3 轮独立复现取中位数（1 KB 消息、每条连接 2000 条、2 vCPU 本机回环）：

| 并发连接 | 聚合吞吐 | 轮间波动 | p50 | p99 | 相对单连接 |
|---|---|---|---|---|---|
...
- **口径**：客户端与服务端同机，客户端同样吃这 2 个 vCPU，因此这是双端同机的聚合值，不等于纯服务端能力上限。
...
bash scripts/bench_wss_concurrent.sh 32 2000 1024   # WSS 并发聚合：[连接数] [每条消息数] [字节数]
```
