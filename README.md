...
项目包含完整的单元测试套件（下表的用例数由 ctest 实跑汇总，`ctest -LE requires-db` 默认配置 12/12、`-DENABLE_WSS=ON` 15/15）：
...
#### 实测结果（2026-09-18，启用 HTTP 长连接后）

> 环境：本机 **2 vCPU / 1968 MB**（**压测端与被测服务同机环回**），Linux 6.8.0，`wrk 4.2.0`（`-t2`，压测线程数与核数相当）
> 服务：`bench_server`（2 个子 Reactor，服务线程池大小见「条件」列）；**HTTP 长连接已启用**，wrk 默认复用连接
> 原始输出：`results/feature-websocket/`（每项文件含环境、工具、时间与 wrk 完整输出）

| 指标 | 数值 | 条件 |
|------|------|------|
| 纯文本吞吐 | **34,119 req/s** | 100 并发 / 30s / 线程池 4 |
| 1000 并发吞吐 | 32,241 req/s | 1000 并发 / 30s |
| JSON 响应吞吐 | 35,217 req/s | 100 并发 / 30s |
| 延迟分布 | **p50 2.75ms / p75 3.51ms / p90 4.42ms / p99 6.35ms** | 100 并发 / 30s |
| 每请求 CPU 时间 | **38.2 us** | 500 并发 / 30s（`/proc/<pid>/stat` 差分 / 请求数） |
| 常驻内存 | 4.2 MB（空闲）到 6.0 MB（满载） | 500 并发 / 60s（VmRSS） |
| 错误率 | 约 0（509,298 请求中 1 次 read error） | 长连接消除了短连接下的 read 误报 |

**内存池对照实验结论（含调用频率实测）**：长连接 + 500 并发压测期间累计 **1,008,350 请求**，
内存池的 `allocate/deallocate` 调用仅增加 **1,002 次**（500 个连接各分配 2 个缓冲后完全复用），
即**每千请求约 1 次分配**（可用 `curl /stats` 读到 `mempool.alloc_calls` 复现）。因此：

- 池的调用频率已低到不可能带来可观测收益，**去锁 / thread-local 化等进一步优化不值得投入**；
  内存池作为**可选特性**保留，默认关闭——在 glibc `tcache`（per-thread 无锁缓存）面前它没有优势。
- 但 `PooledBuffer::clear()` 原本每次清零 12KB 的 `memset` 是纯浪费（长连接下每请求触发两次：
  读完后全消费 + 响应发完后重置），移除后**所有对照组合提升 5.5%~32%**（内存池 ON 组提升最大）。

**并发梯度**（HTTP 长连接，`wrk -t4 -d30s --latency`，服务线程池 4，同机环回）：

| 并发 | 吞吐 (req/s) | p50 | p75 | p90 | p99 | 错误 |
|------|-------------|-----|-----|-----|-----|------|
| 100 | **37,974** | 2.52ms | 3.12ms | 3.78ms | 5.78ms | 0 |
| 500 | 37,308 | 13.07ms | 15.47ms | 18.09ms | 23.84ms | 0 |
| 1000 | 36,143 | 26.98ms | 31.32ms | 36.06ms | 46.27ms | 0 |
| 2000 | 33,609 | 58.41ms | 65.79ms | 73.76ms | 93.49ms | 0 |
| 5000 | **34,245** | 141.30ms | 161.18ms | 180.81ms | 216.39ms | 0 |

**读法**：吞吐在 100–1000 并发区间稳定在 36,000–38,000 req/s；**5,000 并发下仍保持 34,245 req/s 且零错误**，
说明连接管理在高压下稳定。延迟随并发近似线性增长（符合 `延迟 ≈ 并发 ÷ 吞吐` 的排队关系），
因此**「低延迟」与「高并发」必须同时标注档位**才有意义。原始输出：`results/feature-websocket/a7_concurrency.txt`。

两项**与早期数据相反**的现象（如实记录，供后续分析）：

1. **内存池在小核数 + 长连接形态下反而降低吞吐**：OFF 32,362 对 ON 26,732 req/s（下降 17%）。
   早期"内存池提升吞吐 55%"来自短连接形态（每请求新建/销毁缓冲）；长连接下缓冲复用度提高，池管理开销可能转为净负担。
2. **服务线程池越小越快**：1 线程 52,789 / 2 线程 44,025 / 4 线程 35,085 / 8 线程 33,277 / 16 线程 34,452 req/s。
   2 核机器上线程池扩张带来的跨线程唤醒（eventfd）与锁竞争开销超过并行收益。
