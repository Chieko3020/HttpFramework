...
static bool test_oversized_header() {
    ...
    http::HttpServer server(18901, 2);
...
static bool test_path_traversal() {
    ...
    http::HttpServer server(18902, 2);
    auto router = std::make_shared<router::Router>();
...
static bool test_malformed_request_line() {
    TEST("畸形请求行不导致服务器崩溃");
    http::HttpServer server(18903, 2);
    auto router = std::make_shared<router::Router>();
    router->get("/test", [](const http::HttpRequest&, http::HttpResponse&){});
    server.setRouter(router);
...
static bool test_empty_request() {
    TEST("空请求不导致服务器崩溃");
    http::HttpServer server(18904, 2);
    auto router = std::make_shared<router::Router>();
    router->get("/test", [](const http::HttpRequest&, http::HttpResponse&){});
    server.setRouter(router);
...
static bool test_is_port_in_use() {
    ...
    http::HttpServer server(18905, 2);
    if (!server.start()) {
        FAIL("服务器启动失败");
    }
    std::this_thread::sleep_for(std::chrono::milliseconds(300));
...
static bool test_port_conflict() {
    ...
    http::HttpServer server1(18906, 2);
    if (!server1.start()) {
        FAIL("首个服务器启动失败");
    }
    std::this_thread::sleep_for(std::chrono::milliseconds(300));

    http::HttpServer server2(18906, 2);
